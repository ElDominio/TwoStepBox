...
void setup() {
  ...
  digitalWrite(LED_BUILTIN, HIGH);  //activate LED output
...
void loop() {
  ...
  if (millis() % 2) {
    positionCheckcount++;
    //Serial.println(positionCheckcount);
  }
  if (positionCheckcount > 1500) {
    oldPotPos = potPosition;
    bInLedState = !bInLedState;
    //if(bInLedState){ digitalWrite(LED_BUILTIN, HIGH); }else{digitalWrite(LED_BUILTIN, LOW);}
    digitalWrite(LED_BUILTIN, bInLedState);
    //potPosition = (8*!digitalRead(pinFour)) + (4*!digitalRead(pinThree)) +(2*!digitalRead(pinTwo)) +(!digitalRead(pinOne)) ;
    potPosition = 0;
 ...
 */
    ...
    while (i < 4) {
      ...
      while (i == 1) {  // enter RPM dialog
        byte readRPM[4];
        byte digits = 0;
        Serial.print(F("La posicion que se esta editando es: "));
      ...
      while (i == 2) {  // enter cut time dialog
        ...
        byte digits = 0;
        Serial.print(F("Los RPM de corte entrados fueron: "));
        Serial.println(cutRPM);
        Serial.println(F("------"));
      ...
      if (i == 3) {
        Serial.print(F("La ventana de corte es: "));
        Serial.println(timeToCut);
    ...
    Serial.print(F("RPM Filter inverted: "));
    Serial.println(bitRead(bitField, BIT_RPM_FILTER));
    ...
    Serial.print(F("Spark mode set! Current mode is: "));
    Serial.println(cylCount);
  }  // end C case
}
...
void loadCalibration() {
  ...
  byte tempPotPos = 0;
  Serial.print(("tempPotPoggs = "));
  Serial.println(tempPotPos);
